...
class GetUserNewsCubit extends Cubit<GetUserNewsState> {
  ...
  void getGetUserNews({required String userId, required String langId, String? latitude, String? longitude}) async {
    try {
      emit(GetUserNewsFetchInProgress());
      final result = await _getUserNewsRepository.getGetUserNews(limit: limitOfAPIData.toString(), offset: "0", userId: userId, langId: langId, latitude: latitude, longitude: longitude);
  ...
  void getMoreGetUserNews({required String userId, required String langId, String? latitude, String? longitude}) async {
    if (state is GetUserNewsFetchSuccess) {
      try {
        final result = await _getUserNewsRepository.getGetUserNews(
            limit: limitOfAPIData.toString(), offset: (state as GetUserNewsFetchSuccess).getUserNews.length.toString(), langId: langId, userId: userId, latitude: latitude, longitude: longitude);
