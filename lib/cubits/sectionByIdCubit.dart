...
class SectionByIdFetchSuccess extends SectionByIdState {
  ...
  SectionByIdFetchSuccess({required this.newsModel, required this.breakNewsModel, required this.totalCount, required this.hasMoreFetchError, required this.hasMore, required this.type});
}
...
class SectionByIdCubit extends Cubit<SectionByIdState> {
  ...
  void getSectionById({required String userId, required String langId, required String sectionId, String? latitude, String? longitude}) async {
    try {
      emit(SectionByIdFetchInProgress());
      final result = await _sectionByIdRepository.getSectionById(userId: userId, langId: langId, sectionId: sectionId, latitude: latitude, longitude: longitude);
  ...
  void getMoreSectionById({required String userId, required String langId, required sectionId, String? latitude, String? longitude}) async {
    if (state is SectionByIdFetchSuccess) {
      try {
        final result = await _sectionByIdRepository.getSectionById(langId: langId, userId: userId, sectionId: sectionId, latitude: latitude, longitude: longitude);
