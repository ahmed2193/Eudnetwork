...
class VideoCubit extends Cubit<VideoState> {
  ...
  void getVideo({required BuildContext context, required String langId, String? latitude, String? longitude}) async {
    try {
      emit(VideoFetchInProgress());
      final result = await _videoRepository.getVideo(limit: limitOfAPIData.toString(), offset: "0", langId: langId, context: context, latitude: latitude, longitude: longitude);
  ...
  void getMoreVideo({required BuildContext context, required String langId, String? latitude, String? longitude}) async {
    if (state is VideoFetchSuccess) {
      try {
        final result = await _videoRepository.getVideo(
            context: context, langId: langId, limit: limitOfAPIData.toString(), offset: (state as VideoFetchSuccess).video.length.toString(), latitude: latitude, longitude: longitude);
