...
class TagNewsCubit extends Cubit<TagNewsState> {
  ...
  void getTagNews({required BuildContext context, required String tagId, required String userId, required String langId, String? latitude, String? longitude}) async {
    try {
      emit(TagNewsFetchInProgress());
      final result = await _tagNewsRepository.getTagNews(context: context, langId: langId, tagId: tagId, userId: userId, latitude: latitude, longitude: longitude);
