...
class SubCatNewsCubit extends Cubit<SubCatNewsState> {
  ...
  void getSubCatNews({required BuildContext context, String? catId, String? subCatId, String? latitude, String? longitude, required String userId, required String langId}) async {
    try {
      emit(SubCatNewsFetchInProgress());
      final result = await _subCatNewsRepository.getSubCatNews(
          limit: limitOfAPIData.toString(), offset: "0", context: context, subCatId: subCatId, userId: userId, langId: langId, catId: catId, latitude: latitude, longitude: longitude);
  ...
  void getMoreSubCatNews({required BuildContext context, String? catId, String? subCatId, String? latitude, String? longitude, required String userId, required String langId}) async {
    if (state is SubCatNewsFetchSuccess) {
      try {
        final result = await _subCatNewsRepository.getSubCatNews(
            context: context,
